...
// ---------------------------------------------------------------------------
// Country-level cache (World Bank, IMF, GDELT) — 24h TTL
// NASA & AQICN are coordinate-based so they are NOT cached here.
// ---------------------------------------------------------------------------

const COUNTRY_CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
const countryCache = new Map(); // key → { data, fetchedAt }

async function withCountryCache(key, fetcher) {
  const hit = countryCache.get(key);
  if (hit && Date.now() - hit.fetchedAt < COUNTRY_CACHE_TTL_MS) {
    console.log(`[RealData][cache hit] ${key}`);
    return hit.data;
  }
  const data = await fetcher();
  countryCache.set(key, { data, fetchedAt: Date.now() });
  return data;
}
...
export async function fetchWorldBankData(iso2) {
  return withCountryCache(`worldbank:${iso2}`, () => _fetchWorldBankData(iso2));
}

async function _fetchWorldBankData(iso2) {
  try {
    const indicators = [
...
export async function fetchImfData(iso3) {
  return withCountryCache(`imf:${iso3}`, () => _fetchImfData(iso3));
}

async function _fetchImfData(iso3) {
  try {
...
export async function fetchGdeltData(country) {
  return withCountryCache(`gdelt:${country.toLowerCase()}`, () => _fetchGdeltData(country));
}

async function _fetchGdeltData(country) {
  try {
...
export async function fetchEnvironmentalData({ city }) {
  return fetchAqicnData(city);
}

// ---------------------------------------------------------------------------
// Financing Risk — World Bank Global Financial Development Database
// ---------------------------------------------------------------------------

export async function fetchFinancingData(iso2) {
  if (!iso2) return '';
  return withCountryCache(`financing:${iso2}`, () => _fetchFinancingData(iso2));
}

async function _fetchFinancingData(iso2) {
  try {
    const indicators = [
      { id: 'FR.INR.LEND', label: 'Lending interest rate (%)' },
      { id: 'FS.AST.DOMS.GD.ZS', label: 'Domestic credit to private sector (% of GDP)' },
      { id: 'FB.BNK.CAPA.ZS', label: 'Bank capital to assets ratio (%)' },
      { id: 'FB.AST.NPER.ZS', label: 'Bank nonperforming loans to gross loans (%)' },
    ];
    ...
    if (lines.length === 0) return '';
    console.log(`[RealData][fetchFinancingData] ${iso2}: ${lines.length} indicators`);
    return `World Bank Financing Data (${iso2}):\n${lines.join('\n')}`;
  } catch (e) {
    console.warn('[RealData][fetchFinancingData]', e.message);
    return '';
  }
}

// ---------------------------------------------------------------------------
// Security Risk — GDELT (crime/security query) + World Bank WGI governance
// ---------------------------------------------------------------------------

export async function fetchSecurityData(country, iso2) {
  const key = `security:${iso2 ?? country}`;
  return withCountryCache(key, () => _fetchSecurityData(country, iso2));
}

async function _fetchSecurityData(country, iso2) {
  try {
    const [gdeltResult, wbResult] = await Promise.allSettled([
      _fetchGdeltSecurityData(country),
      iso2 ? _fetchWbGovernanceData(iso2) : Promise.resolve(''),
    ]);

    const parts = [];
    if (gdeltResult.status === 'fulfilled' && gdeltResult.value) parts.push(gdeltResult.value);
    if (wbResult.status === 'fulfilled' && wbResult.value) parts.push(wbResult.value);

    if (parts.length === 0) return '';
    console.log(`[RealData][fetchSecurityData] ${country}: ${parts.length} sources`);
    return parts.join('\n\n');
  } catch (e) {
    console.warn('[RealData][fetchSecurityData]', e.message);
    return '';
  }
}

async function _fetchGdeltSecurityData(country) {
  try {
    const query = encodeURIComponent(`${country} crime violence security attack`);
    const url = `https://api.gdeltproject.org/api/v2/doc/doc?query=${query}&mode=artlist&maxrecords=10&format=json`;
    const res = await fetchWithTimeout(url, {}, 10000);
    if (!res.ok) return '';
    const data = await res.json();
    ...
    return `Recent security/crime news (GDELT, last 30 days):\n${lines.join('\n')}`;
  } catch (e) {
    console.warn('[RealData][fetchGdeltSecurityData]', e.message);
    return '';
  }
}

async function _fetchWbGovernanceData(iso2) {
  try {
    const indicators = [
      { id: 'RL.EST', label: 'Rule of Law estimate (WGI, -2.5 to +2.5)' },
      { id: 'CC.EST', label: 'Control of Corruption estimate (WGI, -2.5 to +2.5)' },
    ];
    ...
    if (lines.length === 0) return '';
    return `World Bank Governance Indicators (${iso2}):\n${lines.join('\n')}`;
  } catch (e) {
    console.warn('[RealData][fetchWbGovernanceData]', e.message);
    return '';
  }
}

// ---------------------------------------------------------------------------
// Infrastructure Risk — World Bank API
// ---------------------------------------------------------------------------

export async function fetchInfrastructureData(iso2) {
  if (!iso2) return '';
  return withCountryCache(`infrastructure:${iso2}`, () => _fetchInfrastructureData(iso2));
}

async function _fetchInfrastructureData(iso2) {
  try {
    const indicators = [
      { id: 'EG.ELC.ACCS.ZS', label: 'Access to electricity (% of population)' },
      { id: 'IT.NET.USER.ZS', label: 'Individuals using the Internet (% of population)' },
      { id: 'IS.ROD.PAVE.ZS', label: 'Roads, paved (% of total roads)' },
      { id: 'SH.H2O.BASW.ZS', label: 'People using at least basic drinking water services (%)' },
    ];
    ...
    if (lines.length === 0) return '';
    console.log(`[RealData][fetchInfrastructureData] ${iso2}: ${lines.length} indicators`);
    return `World Bank Infrastructure Data (${iso2}):\n${lines.join('\n')}`;
  } catch (e) {
    console.warn('[RealData][fetchInfrastructureData]', e.message);
    return '';
  }
}
