...
async function runRiskAgent(agentName, researchGoal, promptName, extraContext = '') {
  ...
  try {
    ...
    console.log(`[${agentName}] Empromptu applyPrompt("${promptName}") raw:`, JSON.stringify(result));

    // Parse structured result — expect [{score: N, summary: "..."}]
    if (Array.isArray(result) && result.length > 0) {
      const parsed = result[0];
      const rawScore = parsed.score ?? parsed.risk_score ?? parsed.value;
      const parsedScore = rawScore === undefined || rawScore === null ? NaN : parseInt(rawScore, 10);
      const nextScore = Number.isFinite(parsedScore) ? Math.min(100, Math.max(0, parsedScore)) : 50;
      console.log(
        `[${agentName}] Empromptu extracted score fields:`,
        JSON.stringify({ score: parsed?.score, risk_score: parsed?.risk_score, value: parsed?.value }),
        `=> final_score=${nextScore}`
      );
      score = nextScore;
      summary = parsed.summary ?? '';
      if (score === 50 && !Number.isFinite(parsedScore)) {
        console.warn(`[${agentName}] Using fallback score=50 (unparseable/missing score from Empromptu).`);
      }
    } else if (typeof result === 'string') {
      const match = result.match(/\b(\d{1,3})\b/);
      if (match) {
        score = Math.min(100, Math.max(0, parseInt(match[1], 10)));
        console.log(`[${agentName}] Empromptu string score match="${match[1]}" => final_score=${score}`);
      } else {
        console.warn(`[${agentName}] Empromptu returned string without numeric score; using fallback score=50.`);
      }
    } else if (typeof result === 'number') {
      score = Math.min(100, Math.max(0, Math.round(result)));
      console.log(`[${agentName}] Empromptu numeric result=${result} => final_score=${score}`);
    } else {
      console.warn(`[${agentName}] Empromptu returned unexpected type (${typeof result}); using fallback score=50.`);
    }
  } catch (e) {
    console.error(`${agentName} scoring error (Empromptu applyPrompt failed):`, e.message);
    score = 50;
  }

  return { score, summary, research: researchData };
}
...
export async function runRiskAssessment(property, onProgress, reportId) {
  ...
  await Promise.all(
    agents.map(async (agent) => {
      ...
      const { score, summary, research } = await runRiskAgent(
        agent.name,
        agent.goal,
        agent.prompt,
        agent.extraContext
      ...
      rawData[agent.key] = { score, summary, research };
